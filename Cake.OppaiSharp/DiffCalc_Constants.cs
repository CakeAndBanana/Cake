...
namespace OppaiSharp
{
    public partial class DiffCalc
    {
        ...
        private static readonly Dictionary<StrainType, double> DecayBase = new Dictionary<StrainType, double>
            {{StrainType.Speed, 0.3}, {StrainType.Aim, 0.15}};

        /// <summary>balances speed and aim</summary>
        private static readonly Dictionary<StrainType, double> WeightScaling = new Dictionary<StrainType, double>
            {{StrainType.Speed, 1400.0}, {StrainType.Aim, 26.25}};

        private const double MaxSpeedBonus = 45.0; /* ~330BPM 1/4 streams */

        private const double MinSpeedBonus = 75.0; /* ~200BPM 1/4 streams */

        private const double AngleBonusScale = 90;

        private const double AimTimingThreshold = 107;

        private const double SpeedAngleBonusBegin = 5 * Math.PI / 6;

        private const double AimAngleBonusBegin = Math.PI / 3;
    }
}